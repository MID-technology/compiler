...
void printExpression(const Expression* expr, int indent) {
    if (auto* intLit = dynamic_cast<const IntegerLiteral*>(expr)) {
    ...
    } else if (auto* thisExpr = dynamic_cast<const ThisExpression*>(expr)) {
        printIndent(indent);
        ...
        std::cout << "Object:\n";
        printExpression(memberAccess->object.get(), indent + 2);
        printIndent(indent + 1);
        std::cout << "Member: " << memberAccess->member->name << "\n";
    } else if (auto* funcCall = dynamic_cast<const FunctionCall*>(expr)) {
        printIndent(indent);
        std::cout << "FunctionCall:\n";
        printIndent(indent + 1);
        std::cout << "Callee:\n";
        printExpression(funcCall->callee.get(), indent + 2);
        printIndent(indent + 1);
        std::cout << "Arguments:\n";
        for (const auto& arg : funcCall->arguments) {
            printExpression(arg.get(), indent + 2);
        }
    } else if (auto* ctorInvoc = dynamic_cast<const ConstructorInvocation*>(expr)) {
        printIndent(indent);
        std::cout << "ConstructorInvocation: " << ctorInvoc->className->name << "\n";
        if (!ctorInvoc->arguments.empty()) {
            printIndent(indent + 1);
            std::cout << "Arguments:\n";
            for (const auto& arg : ctorInvoc->arguments) {
                printExpression(arg.get(), indent + 2);
            }
        }
    }
